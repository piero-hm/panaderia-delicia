        ...
        <div className="flex flex-1 flex-col space-y-2 p-4">
          ...
          <div className="flex flex-1 flex-col justify-end mt-auto">
            <div className="flex items-baseline mt-2">
              {product.discount_price ? (
                <>
                  <p className="text-md font-bold text-red-600">{`S/ ${product.discount_price.toFixed(2)}`}</p>
                  <p className="text-sm text-gray-500 line-through ml-2">{`S/ ${product.price.toFixed(2)}`}</p>
                </>
              ) : (
                <p className="text-md font-bold text-gray-800">{`S/ ${product.price.toFixed(2)}`}</p>
              )}
            </div>
          </div>
        </div>
